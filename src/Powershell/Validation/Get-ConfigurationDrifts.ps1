...
# --- Main Script Logic ---
try {
    Write-ActivityLog "Starting Get-ConfigurationDrifts script."
    Write-ActivityLog "Parameters: BaselinePath='$BaselinePath', ServerName='$ServerName', TestConfigurationDriftPath='$TestConfigurationDriftPath'"
    ...
    if ([string]::IsNullOrWhiteSpace($TestConfigurationDriftPath)) {
        try {
            ...
            if ([string]::IsNullOrEmpty($ScriptDirectory)) {
                ...
                Write-ActivityLog "PSScriptRoot was not available. Using Get-Location: $ScriptDirectory" -Level "DEBUG"
            }
            ...
            Write-ActivityLog "Resolved TestConfigurationDriftPath to: $TestConfigurationDriftPath"
        } catch {
             Write-ActivityLog "Could not automatically resolve path to Test-ConfigurationDrift.ps1. Error: $($_.Exception.Message)" -Level "ERROR"
    ...
    if (-not (Test-Path -Path $TestConfigurationDriftPath -PathType Leaf)) {
        Write-ActivityLog "Test-ConfigurationDrift.ps1 not found at the specified path: $TestConfigurationDriftPath" -Level "ERROR"
        throw "Test-ConfigurationDrift.ps1 not found at path: $TestConfigurationDriftPath"
    }

    Write-ActivityLog "Found Test-ConfigurationDrift.ps1 at: $TestConfigurationDriftPath"
    ...
    if ($PSBoundParameters.ContainsKey('BaselinePath')) { # Only pass BaselinePath if it was provided to this script
        ...
        Write-ActivityLog "Passing BaselinePath '$BaselinePath' to Test-ConfigurationDrift.ps1"
    }
    # Note: The LogPath for Test-ConfigurationDrift.ps1 is managed by itself with its own default.

    Write-ActivityLog "Executing Test-ConfigurationDrift.ps1..."
    $driftTestData = & $TestConfigurationDriftPath @paramsForTestScript

    if ($driftTestData) {
        Write-ActivityLog "Successfully executed Test-ConfigurationDrift.ps1 and received data."
        ...
        # Example: if ($driftTestData.DriftDetected) { Write-ActivityLog "Drift was detected by Test-ConfigurationDrift.ps1" -Level "WARNING" }
    } else {
        Write-ActivityLog "Test-ConfigurationDrift.ps1 executed but returned no data or null." -Level "WARNING"
        # Depending on requirements, this might be an error or just an indication of no drift / no checks performed.
    }

    Write-ActivityLog "Get-ConfigurationDrifts script finished."
    return $driftTestData

}
catch {
    Write-ActivityLog "An error occurred in Get-ConfigurationDrifts script: $($_.Exception.Message)" -Level "FATAL"
