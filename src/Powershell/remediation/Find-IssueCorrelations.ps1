...
Function Find-IssueCorrelations {
    ...
    Write-ActivityLog "Starting Find-IssueCorrelations script. InputEvents count: $($InputEvents.Count)."
    Write-ActivityLog "Params: CorrelationTimeWindowSeconds=$CorrelationTimeWindowSeconds, PrimaryIssueIdPattern='$PrimaryIssueIdPattern', MinCorrelationCount=$MinCorrelationCount."

    if ($InputEvents.Count -lt 2) {
        Write-ActivityLog "Not enough input events (need at least 2) to find correlations. Exiting." -Level "WARNING"
        return @()
    }
    ...
    foreach ($inputEvent in $InputEvents) {
        $timestamp = $null
        try {
            if ($inputEvent.PSObject.Properties['Timestamp']) {
                $timestamp = [datetime]$inputEvent.Timestamp
            } else {
                Write-ActivityLog "Input event missing 'Timestamp' property. $($inputEvent | Out-String -Depth 1)" -Level "WARNING"
                continue
            }
        } catch {
            Write-ActivityLog "Failed to convert Timestamp to DateTime for an event. Error: $($_.Exception.Message). Event: $($inputEvent | Out-String -Depth 1)" -Level "WARNING"
            continue
        }
        ...
        $identifier = $null
        if ($inputEvent.PSObject.Properties['MatchedIssueId']) { $identifier = $inputEvent.MatchedIssueId }
        elseif ($inputEvent.PSObject.Properties['IssueId']) { $identifier = $inputEvent.IssueId }
        elseif ($inputEvent.PSObject.Properties['RootCauseRuleId']) { $identifier = $inputEvent.RootCauseRuleId }
        elseif ($inputEvent.PSObject.Properties['RecommendationId']) { $identifier = $inputEvent.RecommendationId }
        elseif ($inputEvent.PSObject.Properties['SuggestedRemediationId']) { $identifier = $inputEvent.SuggestedRemediationId }
        elseif ($inputEvent.PSObject.Properties['RemediationActionId']) { $identifier = $inputEvent.RemediationActionId }
        elseif ($inputEvent.PSObject.Properties['PatternName']) { $identifier = $inputEvent.PatternName }
        elseif ($inputEvent.PSObject.Properties['EventId'] -and $inputEvent.PSObject.Properties['Source']) { $identifier = "Event:$($inputEvent.Source):$($inputEvent.EventId)" }
        elseif ($inputEvent.PSObject.Properties['Source'] -and $inputEvent.PSObject.Properties['Message']) { $identifier = "Source:$($inputEvent.Source):MsgSubstr:$($inputEvent.Message.Substring(0, [System.Math]::Min($inputEvent.Message.Length, 20)))" } # Fallback
    ...
    if ($processedEvents.Count -lt 2) {
        Write-ActivityLog "Not enough processable input events (with valid Timestamps and Identifiers) to find correlations. Exiting." -Level "WARNING"
        return @()
    }

    # Sort by Timestamp
    $sortedEvents = $processedEvents | Sort-Object Timestamp
    Write-ActivityLog "Processed and sorted $($sortedEvents.Count) events."
    ...
    if ([string]::IsNullOrWhiteSpace($effectivePrimaryIssueIdPattern)) {
        Write-ActivityLog "PrimaryIssueIdPattern not specified. Identifying most frequent event type to use as primary."
        ...
        if ($mostFrequent) {
            ...
            Write-ActivityLog "Using '$effectivePrimaryIssueIdPattern' (Count: $($mostFrequent.Count)) as the implicit primary event type for correlation."
        } else {
            Write-ActivityLog "Could not determine a most frequent event type. Cannot proceed with 'primary event' correlation logic without a PrimaryIssueIdPattern." -Level "ERROR"
            return @()
        }
    }
    ...
    Write-ActivityLog "Finding correlations around primary event type: '$effectivePrimaryIssueIdPattern'"

    $primaryEvents = $sortedEvents | Where-Object { $_.Identifier -eq $effectivePrimaryIssueIdPattern }

    if ($primaryEvents.Count -eq 0) {
        Write-ActivityLog "No instances of primary event type '$effectivePrimaryIssueIdPattern' found in the input data." -Level "WARNING"
        return @()
    }
    Write-ActivityLog "Found $($primaryEvents.Count) instances of the primary event type '$effectivePrimaryIssueIdPattern'."
    ...
    foreach ($pEvent in $primaryEvents) {
        ...
        Write-ActivityLog "Analyzing window for primary event at $($pEvent.Timestamp): $windowStart to $windowEnd" -Level "DEBUG"
        ...
        foreach ($sEvent in $eventsInWindow) {
            ...
            Write-ActivityLog "Found potential correlation: Key='$correlationKey', pEventTime='$($pEvent.Timestamp)', sEventTime='$($sEvent.Timestamp)'" -Level "DEBUG"
        }
    }
    ...
    foreach ($key in $correlations.Keys) {
        if ($correlations[$key] -ge $MinCorrelationCount) {
            ...
            $outputCorrelations.Add([PSCustomObject]@{
                ...
                ExamplePrimaryEventTimestamps = ($primaryEventOccurrences | Select-Object -Unique | Sort-Object | Select-Object -First 5)
            }) | Out-Null
            Write-ActivityLog "Significant correlation found: '$key', Count: $($correlations[$key])"
        }
    }

    Write-ActivityLog "Find-IssueCorrelations script finished. Found $($outputCorrelations.Count) significant correlations meeting MinCorrelationCount=$MinCorrelationCount."
