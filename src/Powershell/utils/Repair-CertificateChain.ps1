...
Function Repair-CertificateChain {
    ...
    Write-ActivityLog "Starting Repair-CertificateChain for certificate: Subject='$($Certificate.Subject)', Thumbprint='$($Certificate.Thumbprint)'."
    Write-ActivityLog "Parameters: InstallMissingIntermediates='$InstallMissingIntermediates'."
    ...
    if ($InstallMissingIntermediates -and $IntermediateStoreLocation -eq "LocalMachine") {
        if (-not ([Security.Principal.WindowsPrincipal][Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)) {
            Write-ActivityLog "Administrator privileges are required to install intermediate certificates to LocalMachine store. AIA fetching and installation will be skipped if needed." -Level "WARNING"
    ...
    try {
        Write-ActivityLog "Performing initial chain build..."
        ...
        Write-ActivityLog "Initial chain build attempt completed. Success: $initialChainBuilt. Status: $(Get-ChainStatusStrings $chain.ChainStatus)"

        if ($chain.ChainElements.Count -gt 0) {
            Write-ActivityLog "Initial chain elements ($($chain.ChainElements.Count)): "
            $chain.ChainElements | ForEach-Object { Write-ActivityLog "  Subject: $($_.Certificate.Subject), Issuer: $($_.Certificate.Issuer), Status: $(Get-ChainStatusStrings $_.ChainElementStatus)" -Level "DEBUG" }
        }
        ...
        if (-not $initialChainBuilt -and $InstallMissingIntermediates -and ($initialChainStatus -contains "PartialChain" -or $initialChainStatus -contains "OfflineRevocation")) {
            Write-ActivityLog "Initial chain is incomplete. Attempting to download and install missing intermediates from AIA."
            ...
            for ($i = 0; $i -lt ($chain.ChainElements.Count - 1); $i++) {
                ...
                if ($elementStatus -contains "PartialChain" -or $elementStatus -contains "OfflineRevocation") {
                    Write-ActivityLog "Chain element '$($elementCert.Subject)' has status indicating potential missing issuer. Checking AIA." -Level "DEBUG"
                    ...
                    if ($aiaExtension) {
                        ...
                        Write-ActivityLog "AIA extension found for '$($elementCert.Subject)': $aiaFormatted" -Level "DEBUG"
                        ...
                        foreach ($url in $aiaUrls) {
                            Write-ActivityLog "Found CA Issuers AIA URL: $url"
                            ...
                            if ($PSCmdlet.ShouldProcess($url, "Download Potential Intermediate Certificate")) {
                                try {
                                    Write-ActivityLog "Attempting to download from '$url' to '$tempCertPath'."
                                    Invoke-WebRequest -Uri $url -OutFile $tempCertPath -TimeoutSec 10 -ErrorAction Stop
                                    Write-ActivityLog "Downloaded successfully."
                                    $downloadStatus = "Success"

                                    if (-not (Test-Path $PathInstallIntermediateCerts -PathType Leaf)) {
                                        Write-ActivityLog "Install-IntermediateCertificates.ps1 not found at '$PathInstallIntermediateCerts'. Cannot install downloaded certificate." -Level "ERROR"
                                    ...
                                    Write-ActivityLog "Attempting to install '$tempCertPath' to $IntermediateStoreLocation\$IntermediateStoreName."
                                    ...
                                    if ($installResult -and $installResult[0].Status -eq "Success") {
                                        ...
                                        Write-ActivityLog "Successfully installed certificate from '$url' (Thumbprint: $downloadedCertThumbprint)."
                                    } else {
                                        $installStatus = "Failed"
                                        Write-ActivityLog "Failed to install certificate from '$url'. Install script msg: $($installResult[0].ErrorMessage)" -Level "WARNING"
                                    }
                                } catch {
                                    Write-ActivityLog "Failed to download or process certificate from '$url'. Error: $($_.Exception.Message)" -Level "WARNING"
                                ...
                                Write-ActivityLog "Download from '$url' skipped due to -WhatIf."
                    ...
                    } else { Write-ActivityLog "No AIA extension found for '$($elementCert.Subject)'." -Level "DEBUG" }
                } # End if element has partial chain/offline revocation
            } # End for loop through chain elements

            if ($installedSomethingNew) {
                Write-ActivityLog "Re-building chain after attempting intermediate certificate installations..."
                ...
                Write-ActivityLog "Second chain build attempt completed. Success: $initialChainBuilt. Status: $(Get-ChainStatusStrings $chain.ChainStatus)"
                if ($chain.ChainElements.Count -gt 0) {
                    Write-ActivityLog "Final chain elements ($($chain.ChainElements.Count)): "
                    $chain.ChainElements | ForEach-Object { Write-ActivityLog "  Subject: $($_.Certificate.Subject), Issuer: $($_.Certificate.Issuer), Status: $(Get-ChainStatusStrings $_.ChainElementStatus)" -Level "DEBUG" }
                }
            }
        ...
        if ($initialChainBuilt) { # Check the result of the latest chain.Build()
            ...
            Write-ActivityLog "Final chain validation successful."
            ...
            Write-ActivityLog "Final chain validation failed, and no repair was attempted or conditions not met for repair." -Level "WARNING"
             ...
             Write-ActivityLog "Final chain validation after repair attempt: $overallResult" -Level (if($initialChainBuilt){"INFO"}else{"WARNING"})
        }

    } catch {
        Write-ActivityLog "A critical error occurred during chain processing: $($_.Exception.Message)" -Level "FATAL"
        Write-ActivityLog $_.ScriptStackTrace -Level "DEBUG"
    ...
    Write-ActivityLog "Repair-CertificateChain script finished. Overall Result: $overallResult."
    return $finalOutput
}
