...
try {
    ...
    $queries = @(
        ...
        @{
            ...
            Id = 2004
            KeywordsFilter = @('disk space', 'storage')
    ...
    foreach ($query in $queries) {
        ...
        try {
            ...
            if ($query.Id) {
                $filterHashtable.Id = $query.Id
            }

            if ($query.ProviderName) {
                $filterHashtable.ProviderName = $query.ProviderName
            }
            ...
            if ($events) {
                ...
                foreach ($inputEvent in $events) {
                    ...
                    if ($query.Id -eq 2013 -and $inputEvent.ProviderName -match 'srv') {
