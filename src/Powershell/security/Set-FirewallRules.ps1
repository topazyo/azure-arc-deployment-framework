...
# This script configures Windows Firewall rules based on a provided rules object.

param (
    [Parameter(Mandatory = $true)]
    [object]$Rules, # This object should be the firewallRules section from the JSON
...
try {
    ...
    # Define paths - ScriptRoot might still be useful for backup path construction if relative
    $ScriptRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
    # $ConfigFile = Join-Path -Path $ScriptRoot -ChildPath "..\..\config\security-baseline.json" # REMOVED

    # Validate input Rules object
    if (-not $Rules) {
        Write-Log "The -Rules parameter is null. Configuration cannot be applied." -Level "ERROR"
        throw "Rules object not provided."
    }
    # $FirewallSettings will now be the $Rules parameter directly.
    # Write-Log "Using provided firewall rules object." # Optional
    ...
    foreach ($direction in @("outbound", "inbound")) {
        ...
        if ($Rules.$direction) {
            foreach ($ruleDef in $Rules.$direction) {
