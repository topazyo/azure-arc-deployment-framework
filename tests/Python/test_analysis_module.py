...
class TestRootCauseAnalyzer:
    def test_rca_init(self, comprehensive_config):
        ...
        assert rca is not None, "RootCauseAnalyzer failed to instantiate"
        assert isinstance(rca.ml_model, SimpleRCAEstimator), "ml_model must be a SimpleRCAEstimator"
        assert isinstance(rca.explainer, SimpleRCAExplainer), "explainer must be a SimpleRCAExplainer"
        assert isinstance(rca.pattern_analyzer, PatternAnalyzer), "pattern_analyzer must be a PatternAnalyzer"
        assert rca.config is comprehensive_config, "config must be stored as-is"
        assert hasattr(rca, 'logger'), "RootCauseAnalyzer must set up a logger"
...
class TestPatternAnalyzer:
    def test_pa_init(self, comprehensive_config):
        ...
        assert pa is not None, "PatternAnalyzer failed to instantiate"
        assert pa.config is comprehensive_config, "config must be stored as-is"  # Verify config is stored
        assert pa.dbscan_eps == comprehensive_config["dbscan_eps"], "dbscan_eps must match config"

    def test_pa_temporal_patterns(self, comprehensive_config, telemetry_df_for_pattern_analyzer):
        pa = PatternAnalyzer(config=comprehensive_config)
        assert pa.config is comprehensive_config, "config must be wired in"  # Verify config is wired in
        df = telemetry_df_for_pattern_analyzer.copy()

        daily_patterns = pa.analyze_daily_patterns(df)
        assert "peak_hours" in daily_patterns, "daily_patterns must contain peak_hours"
        assert "seasonality_strength" in daily_patterns, "daily_patterns must contain seasonality_strength"
        assert "recommendations" in daily_patterns, "daily_patterns must contain recommendations"

        weekly_patterns = pa.analyze_weekly_patterns(df)
        assert "peak_days" in weekly_patterns, "weekly_patterns must contain peak_days"
        assert "seasonality_strength" in weekly_patterns, "weekly_patterns must contain seasonality_strength"
        assert "recommendations" in weekly_patterns, "weekly_patterns must contain recommendations"

        monthly_patterns = pa.analyze_monthly_patterns(df)
        assert "peak_days_of_month" in monthly_patterns, "monthly_patterns must contain peak_days_of_month"
        assert "recommendations" in monthly_patterns, "monthly_patterns must contain recommendations"

        # Test main temporal aggregator
        temporal_results = pa.analyze_temporal_patterns(df)
        assert "daily" in temporal_results, "temporal_results must contain daily"
        assert "weekly" in temporal_results, "temporal_results must contain weekly"
        assert "monthly" in temporal_results, "temporal_results must contain monthly"
        assert "recommendations" in temporal_results, "temporal_results must contain recommendations"
    ...
    def test_pa_behavioral_patterns(self, comprehensive_config, telemetry_df_for_pattern_analyzer):
        ...
        assert "clusters" in behavioral_results, "behavioral_results must contain clusters"
        assert "recommendations" in behavioral_results, "behavioral_results must contain recommendations"
    ...
    def test_pa_failure_patterns(self, comprehensive_config, telemetry_df_for_pattern_analyzer):
        ...
        assert "common_causes" in common_causes, "common_causes must be present"
        assert "recommendations" in common_causes, "recommendations must be present"
        ...
        assert "precursors" in precursors, "precursors must be present"
        assert "recommendations" in precursors, "recommendations must be present"
        # Add more specific precursor content checks if data is designed for it

        impact = pa.analyze_failure_impact(df)
        assert "average_downtime" in impact, "impact must contain average_downtime"
        assert "recommendations" in impact, "impact must contain recommendations"

        # Test main failure aggregator
        failure_results = pa.analyze_failure_patterns(df)
        assert "common_causes" in failure_results, "failure_results must contain common_causes"
        assert "precursors" in failure_results, "failure_results must contain precursors"
        assert "impact_analysis" in failure_results, "failure_results must contain impact_analysis"
        assert "recommendations" in failure_results, "failure_results must contain recommendations"
    ...
    def test_pa_performance_patterns(self, comprehensive_config, telemetry_df_for_pattern_analyzer):
        ...
        assert "metric_stats" in resource_usage, "resource_usage must contain metric_stats"
        assert "recommendations" in resource_usage, "resource_usage must contain recommendations"

        bottlenecks = pa.identify_bottlenecks(df) # This uses config['bottleneck_rules']
        assert "detected_bottlenecks" in bottlenecks, "bottlenecks must contain detected_bottlenecks"
        assert "recommendations" in bottlenecks, "bottlenecks must contain recommendations"

        perf_trends = pa.analyze_performance_trends(df)
        assert "trends" in perf_trends, "perf_trends must contain trends"
        assert "recommendations" in perf_trends, "perf_trends must contain recommendations"

        # Test main performance aggregator
        performance_results = pa.analyze_performance_patterns(df)
        assert "resource_usage" in performance_results, "performance_results must contain resource_usage"
        assert "bottlenecks" in performance_results, "performance_results must contain bottlenecks"
        assert "trends" in performance_results, "performance_results must contain trends"
        assert "recommendations" in performance_results, "performance_results must contain recommendations"


class TestTelemetryProcessor:
    def test_tp_init(self, comprehensive_config):
        tp = TelemetryProcessor(config=comprehensive_config)
        assert tp is not None, "TelemetryProcessor failed to instantiate"
        assert tp.config is comprehensive_config, "config must be stored as-is"
    ...
    def test_tp_process_telemetry(self, comprehensive_config, telemetry_df_for_processor):
        ...
        assert "processed_data" in processed_output, "processed_output must contain processed_data" # Cleaned DataFrame
        assert "anomalies" in processed_output, "processed_output must contain anomalies"
        assert "trends" in processed_output, "processed_output must contain trends" # This is from _analyze_trends
        assert "insights" in processed_output, "processed_output must contain insights"
        assert isinstance(processed_output["processed_data"], pd.DataFrame), "processed_data must be a DataFrame"
        assert "flattened_features" in processed_output, "processed_output must contain flattened_features"
        assert isinstance(processed_output["flattened_features"], dict), "flattened_features must be a dict"
    ...
    def test_tp_handle_missing_values(self, comprehensive_config, telemetry_df_for_processor):
        ...
        assert tp_mean.config is mean_config, "config must be stored"  # Verify custom config is stored
        df_mean_filled = tp_mean._handle_missing_values(telemetry_df_for_processor.copy())
        assert not df_mean_filled['disk_usage'].isnull().any(), "disk_usage should be filled" # Was NaN, check if filled
        assert df_mean_filled['categorical_feat'].iloc[2] == 'unknown', "categorical_feat should be filled with 'unknown'" # Was NaN
        ...
        assert not df_median_filled['disk_usage'].isnull().any(), "disk_usage should be filled with median"
        ...
        assert (df_zero_filled['disk_usage'] == 0).sum() >= telemetry_df_for_processor['disk_usage'].isnull().sum(), "NaNs should be replaced with 0"
    ...
    def test_tp_prepare_feature_matrix(self, comprehensive_config, telemetry_df_for_processor):
        ...
        assert matrix.shape[1] == len(comprehensive_config['anomaly_detection_features']), "matrix columns must match feature count"
        assert names == comprehensive_config['anomaly_detection_features'], "names must match configured features"
        ...
        assert matrix[0,0] == pytest.approx(0.75), "First feature value must match"
        assert matrix[0,1] == pytest.approx(0.65), "Second feature value must match"
        assert matrix[0,2] == pytest.approx(0.05), "Third feature value must match"
    ...
    def test_tp_get_anomalous_features(self, comprehensive_config):
        ...
        assert len(anom_feats) == len(feature_names), "anom_feats length must match feature_names"
        assert anom_feats["cpu"] == pytest.approx(0.8), "cpu anomalous feature value must match"
    ...
    def test_tp_calculate_period_trends(self, comprehensive_config, telemetry_df_for_processor):
        tp = TelemetryProcessor(config=comprehensive_config)
        assert tp.config is comprehensive_config, "config must be wired in"  # Verify config is wired in
        df = telemetry_df_for_processor.copy()
        # Ensure 'trend_features' from config are present in df
        trends = tp._calculate_period_trends(df)
        assert "cpu_usage_avg" in trends, "trends must contain cpu_usage_avg"
        assert "response_time_p95" in trends, "trends must contain response_time_p95"
        assert "slope" in trends["cpu_usage_avg"], "cpu trend must contain slope"
        assert "p_value" in trends["cpu_usage_avg"], "cpu trend must contain p_value"
        if trends["cpu_usage_avg"]["p_value"] < comprehensive_config.get("trend_p_value_threshold", 0.05):
            assert trends["cpu_usage_avg"]["significant"] == True, "trend should be marked significant"

        # Test with insufficient data
        trends_short = tp._calculate_period_trends(df.head(2)) # Needs 3 for linregress
        assert trends_short["cpu_usage_avg"]["p_value"] == pytest.approx(1.0), "p_value should be 1.0 for insufficient data" # Default for insufficient
        assert trends_short["cpu_usage_avg"]["significant"] == False, "trend should not be significant with insufficient data"
    ...
    def test_tp_detect_periodic_patterns(self, comprehensive_config, telemetry_df_for_processor):
        ...
        assert "cpu_usage_avg" in patterns, "patterns must detect cpu_usage_avg"
    ...
    def test_tp_detect_correlations(self, comprehensive_config, telemetry_df_for_processor):
        ...
        assert "significant_pairs" in correlations, "correlations must contain significant_pairs"
        assert len(correlations["significant_pairs"]) > 0, "should detect at least one correlation pair"
        found_pair = False
        for p_info in correlations["significant_pairs"]:
            if ("cpu_usage_avg", "memory_usage_avg_corr") == p_info["pair"] or \
               ("memory_usage_avg_corr", "cpu_usage_avg") == p_info["pair"]:
                found_pair = True
                assert abs(p_info["correlation_coefficient"]) > config_with_new_col["correlation_threshold"], "correlation must exceed threshold"
                break
        assert found_pair, "should detect the highly correlated pair"
    ...
    def test_tp_detect_anomalous_patterns(self, comprehensive_config, telemetry_df_for_processor):
        ...
        assert "HighCpuAndErrors" in anom_patterns, "should detect HighCpuAndErrors pattern"
        assert anom_patterns["HighCpuAndErrors"]["count"] >= 1, "pattern count should be at least 1"
...
class TestPatternAnalyzer_LegacyPlaceholderTests:
    ...
    def test_pa_analyze_patterns(self, comprehensive_config, telemetry_df_for_pattern_analyzer):
        ...
        assert "temporal" in patterns, "patterns must contain temporal"
        assert "behavioral" in patterns, "patterns must contain behavioral"
        assert "failure" in patterns, "patterns must contain failure"
        assert "performance" in patterns, "patterns must contain performance"
        # Check for recommendations key in sub-patterns
        assert "recommendations" in patterns.get("temporal", {}), "temporal must contain recommendations" # Top level temporal recommendations
        assert "recommendations" in patterns.get("failure", {}), "failure must contain recommendations"
        assert "recommendations" in patterns.get("performance", {}), "performance must contain recommendations"
    ...
    def test_pa_individual_pattern_methods(self, comprehensive_config, telemetry_df_for_pattern_analyzer):
        ...
        assert isinstance(pa.analyze_daily_patterns(df), dict), "daily_patterns must return dict"
        assert isinstance(pa.analyze_weekly_patterns(df), dict), "weekly_patterns must return dict"
        assert isinstance(pa.analyze_monthly_patterns(df), dict), "monthly_patterns must return dict"

        # These methods now return Dicts with 'causes'/'precursors' as keys to lists
        assert isinstance(pa.identify_common_failure_causes(df), dict), "common_failure_causes must return dict"
        assert "common_causes" in pa.identify_common_failure_causes(df), "common_causes key must be present"

        assert isinstance(pa.identify_failure_precursors(df), dict), "failure_precursors must return dict"
        assert "precursors" in pa.identify_failure_precursors(df), "precursors key must be present"

        assert isinstance(pa.analyze_failure_impact(df), dict), "failure_impact must return dict"
        assert "average_downtime" in pa.analyze_failure_impact(df), "average_downtime key must be present"

        assert isinstance(pa.analyze_resource_usage_patterns(df), dict), "resource_usage_patterns must return dict"
